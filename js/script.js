...
document.addEventListener('DOMContentLoaded', () => {
    ...
    visitorForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const docRef = await db.collection('visitors').add({
                ...
                likes: [] // **السطر الجديد: إضافة مصفوفة الإعجابات الفارغة**
            });
    ...
    registerForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            // **تعديل هنا: إنشاء بريد إلكتروني تلقائي**
            const userEmail = `${registerName.replace(/\s+/g, '').toLowerCase()}@chat.com`;

            // إضافة بيانات المستخدم إلى Firestore
            const docRef = await db.collection('users').add({
                username: registerName,
                password: registerPassword,
                email: userEmail, // **السطر الجديد: إضافة البريد الإلكتروني التلقائي**
                age: registerAge,
        ...
        try {
            ...
            if (snapshot.empty) {
                ...
                if (userData.password === memberPassword) {
                    ...
                    // تحديث بيانات المستخدم عند تسجيل الدخول إذا كان حقل 'likes' غير موجود
                    if (!userData.likes) {
                        await db.collection('users').doc(snapshot.docs[0].id).update({
                            likes: []
                        });
                        console.log('تم تحديث حقل الإعجابات للمستخدم عند تسجيل الدخول.');
                    }
