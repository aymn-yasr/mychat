...
// استيراد دوال Firebase Firestore
import { collection, getDocs, doc, setDoc, updateDoc, onSnapshot, query, where, getDoc, orderBy } from 'https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js';

// استيراد إعدادات Firebase
import { db, serverTimestamp, } from './firebase-config.js';

// استيراد الدوال الأخرى
import { createMessageElement, createSystemMessageElement, addWelcomeMessageToChat, activeQuoteData, hideActiveQuoteBubble, updatePrivateButtonNotification, updatePrivateChatNotification, currentOpenPrivateChatId } from './chat-ui.js';
...
// دالة جلب الغرف باستخدام النمط القديم (إصدار 8)
...
export async function sendMessage(messageText, roomId, imageUrl = null) {
  ...
  // **التعديل هنا:**
  // 1. جلب بيانات المستخدم كاملة من Firestore بدلاً من localStorage
  ...
  const userData = userDoc.data();
  const currentUserName = userData.username; // استخدام الاسم المحدث من قاعدة البيانات
  const currentUserAvatar = userData.avatar; // استخدام الصورة المحدثة
...
export async function updateUserData(userId, dataToUpdate) {
    try {
        ...
        // **السطر الجديد: تحديث الاسم في localStorage إذا تم تغييره**
        ...
        // **السطر الجديد: تحديث الأفاتار في localStorage إذا تم تغييره**
...
// ... (تأكد من وجود الاستيرادات الصحيحة في الأعلى) ...

//js/chat-firestore.js

// ... (تأكد من وجود جميع الاستيرادات في الأعلى)
// تأكد أن هذا الاستيراد صحيح لـ v8 أو قم بتحديثه لـ v9

// الدالة المسؤولة عن الاستماع للإشعارات الخاصة
export function setupPrivateMessageNotificationListener(currentUserId) {
    ...
    db.collection('privateChats').onSnapshot(snapshot => {
        ...
        snapshot.docs.forEach(doc => {
            ...
            // **4. المستمع يتحقق مما إذا كانت المحادثة مفتوحة قبل إضافة العدد**
            // هذا المنطق يمنع المستمع من إضافة إشعار لمحادثة مفتوحة
            const otherUserId = doc.id.replace(currentUserId, '').replace('_', '');
        ...
        // **5. المستمع يقرر ما إذا كان سيظهر الإشعار بناءً على العدد الإجمالي**
