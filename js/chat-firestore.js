...
export function setupRealtimeMessagesListener(roomId) {
  ...
  // استخدام متغير لتجنب مسح الدردشة عند كل تعديل بسيط
  let isFirstSnapshot = true;

  db.collection('rooms').doc(roomId).collection('messages').orderBy('timestamp', 'asc').onSnapshot(async snapshot => {
    // إزالة رسالة التحميل في أول لقطة
    if (isFirstSnapshot) {
      chatBox.innerHTML = '';
    }

    // إذا كانت اللقطة فارغة في البداية، اعرض رسالة
    ...
    const lookupPromises = Array.from(senderIdsToLookup).map(async (senderId) => {
      // (كود جلب بيانات المستخدمين كما هو)
    ...
    snapshot.docChanges().forEach(change => {
      ...
      if (change.type === 'added') {
        if (!existingMessageElement) {
          let messageElement;
          // **الشرط الجديد والمصحح لرسائل النظام**
