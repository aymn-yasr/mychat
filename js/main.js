...
import { setupRealtimeMessagesListener, sendMessage, getPrivateChatContacts, getAllUsersAndVisitors, getUserData, setupPrivateMessageNotificationListener, sendJoinMessage, deleteChatRoomMessages, sendSystemMessage } from './chat-firestore.js';
...
const plusButton = document.querySelector('#input-bar .plus-btn-circle');
const imageUploadInput = document.getElementById('image-upload-input');

if (plusButton && imageUploadInput) {
    plusButton.addEventListener('click', () => {
        // عند النقر على زر +, يتم تشغيل مدخل الملفات المخفي
    ...
    imageUploadInput.addEventListener('change', async (event) => {
        ...
        // عرض نافذة تأكيد للمستخدم قبل الرفع
        if (!confirm) {
            // إذا ألغى المستخدم، قم بتفريغ المدخل
            imageUploadInput.value = '';
            return;
        }

        // داخل معالج حدث تغيير مدخل الملفات
try {
    ...
    if (imageUrl) {
        ...
        scrollToBottom(); // **استدعاء الدالة الجديدة هنا**
    }
} catch (error) {
...
const handleMessageSend = async () => {
    ...
    if (messageText.toLowerCase() === '/clear') {
        if (currentUserRank === 'المالك' || currentUserRank === 'اونر اداري') {
            try {
                await deleteChatRoomMessages(currentRoomId);
                
                // 2. إنشاء رسالة تأكيد نظام جديدة وإرسالها
                ...
                await sendSystemMessage(confirmationMessage, currentRoomId); // <--- استدعاء الدالة الجديدة
                
                messageInput.value = '';
            } catch (error) {
                console.error('فشل تنظيف الدردشة أو إرسال رسالة التأكيد:', error);
                alert('فشل تنظيف الدردشة من قاعدة البيانات.');
            }
            return;
        }
    }
...
// **مهم:** يجب أن يكون هذا الكود في مكان يتم تنفيذه بعد تحميل الصفحة بالكامل
// على سبيل المثال، في نهاية ملف main.js
