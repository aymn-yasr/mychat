...
// js/chat-ui.js
// ... (تأكد من وجود المتغيرات الأخرى هنا)
...
export function createUserInfoModal(targetElement, userData, allUsersAndVisitorsData) {
    ...
    event.stopPropagation();
    hideUserInfoModal();
    // استدعاء المودال الجديد لعرض الملف الشخصي
...
export function hideUserInfoModal() {
    if (window.userInfoModal) {
        ...
        const handler = function() {
            ...
            // لا حاجة لإعادة إزالة المستمع هنا
        };
        
        modalToClose.addEventListener('transitionend', handler, { once: true });
        
        // قم بتعيين المتغير إلى null بعد إزالة المستمعين
        window.userInfoModal = null;
    }
}
...
function restorePrivateChatDialog(userData) {
    if (privateChatDialog) {
        ...
        // **التعديل الجديد: إزالة الدائرة الحمراء من الأيقونة المصغرة**
...
export async function createAndShowPrivateChatDialog(targetUserData) {
    ...
    // **1. قم بتعيين معرف المحادثة المفتوحة هنا أولاً**
    currentOpenPrivateChatId = targetUserData.id;

    // **2. قم بإخفاء الإشعار على زر "خاص" الرئيسي فورًا**
    updatePrivateButtonNotification(false);
    
    // وهذا يزيلها من الأيقونة المصغرة إذا كانت موجودة
    ...
    const currentUserId = localStorage.getItem('chatUserId');
    if (currentUserId) {
        // تحديث قاعدة البيانات لإعادة تعيين عدد الرسائل في الخلفية
    ...
    if (privateChatDialog) {
        ...
        privateChatDialog.addEventListener('transitionend', () => {
            if (privateChatDialog) {
                ...
                // **3. قم بإعادة تعيين معرف المحادثة إلى null عند الإغلاق**
...
// ... (تأكد من وجود الدالة mentionUserInInput و createUserInfoModal ودالة showImageInModal)
// ... (كما أضفنا في الرد السابق)
...
export function createMessageElement(messageData) {
    ...
    // **التعديل الجديد: عرض الصورة المرفوعة**
    ...
    // ... (داخل دالة createMessageElement)
if (messageData.text) {
    const isMentioned = 
    ...
    // إذا كان هناك محتوى نصي، قم بإضافته
...
messageItem.innerHTML = `
    ...
    // **التعديل الجديد: معالج حدث النقر على الاسم**
...
export function updatePrivateChatNotification(senderId, senderData) {
    ...
    if (minimizedAvatar) {
        // إذا وُجدت الصورة المصغرة، أضف لها إشعار الرسائل الجديدة.
        minimizedAvatar.classList.add('has-new-messages');
        
        // الأهم: أخفِ إشعار زر "الخاص" في الشريط العلوي لأن الإشعار أصبح على الصورة المصغرة.
        updatePrivateButtonNotification(false);
    } else {
        // إذا لم تكن هناك أيقونة مصغرة للمرسل، أظهر إشعار زر "الخاص" في الشريط العلوي.
...
// دالة جديدة لعرض الصورة في مودال
